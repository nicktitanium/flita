...
def start(message):
    ...
    if result != None:
        ...
        list.update({str(message.from_user.id): 0})
...
def menu(message):
    ...
    list.update({str(message.from_user.id): cnt})
    if message.text != 'Зарегистрироваться':
        ...
        if message.text == 'Встать в очередь':
            if in_queue == 0:
                client.send_message(config_tg.channel_link,
                                    "Новая заявка!\nИмя студента: " + name + "\nКоличество подходов: " + str(list[user_id]))
                cnt += 1
            ...
            name = str(result[0] + ' ' + result[1])
            if in_queue == 1:
                client.send_message(config_tg.channel_link, "Заявка на имя " + name + " отозвана.")
